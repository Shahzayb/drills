...
describe('core schema (e2e)', () => {
  ...
  // Ordered deletes rather than a transaction: PostgresService hands out pooled
  // connections per query, so there is no session to hold a transaction open
  // across. Reverse FK order, because every FK here is NO ACTION — if the order
  // is wrong, this fails loudly instead of silently leaving orphans.
  afterAll(async () => {
    if (ids.message) {
      await db.query('DELETE FROM messages WHERE id = $1', [ids.message]);
    }
    if (ids.conversation) {
    ...
    if (ids.membership) {
      await db.query('DELETE FROM memberships WHERE id = $1', [ids.membership]);
    }
    if (ids.user) {
  ...
  it('inserts one row of each table and reads them back joined', async () => {
    ...
    // org_id is written directly, not reached through the conversation. That is
