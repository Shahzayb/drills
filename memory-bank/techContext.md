...
`src/observability` owns request correlation. One id (`x-request-id`) is generated or accepted at the Next edge and reaches every layer: a functional middleware derives it, sets the response header and enters an `AsyncLocalStorage`; a `LoggingInterceptor` adds the controller/handler/org line; `PostgresService.query()` appends it to the SQL as a trailing `/* rid=… */`. Both apps log structured JSON through pino, and every line carries `time`/`level`/`svc`/`msg`/`rid` with `durMs` and `status` named the same way whichever layer wrote it. `info` is one line per request per service, `debug` is the inside of one; `LOG_LEVEL` is per-service in `docker-compose.yml` and reads the shell first. `pnpm logs:trace <id>` is the reconstruction. Full reasoning and the field set in `plans/2026-08-13_drill-06-request-id-propagation.md`.

OpenTelemetry sits on top of that and is **off unless `OTEL_EXPORTER_OTLP_ENDPOINT` is set**, which is what keeps drill 05's baseline instrument unchanged. `apps/backend/src/tracing.ts` starts a `NodeSDK` with five named instrumentations (`-http`, `-express`, `-nestjs-core`, `-pg`, `-ioredis`); `apps/frontend/instrumentation.node.ts` starts the same SDK with **no** instrumentations, because Next emits its own spans through `@opentelemetry/api` and the only missing piece is the outgoing `traceparent`, injected by hand in `lib/api.ts`. Spans go to an OTLP collector (`otel/collector.yaml`) and on to Jaeger, both under `profiles: ['trace']`. When tracing is on, every log line gains `trace_id`/`span_id` (snake_case deliberately — that is what Grafana/Loki/Datadog key trace links off) and the hand-rolled SQL comment stands down in favour of instrumentation-pg's sqlcommenter. What a trace answers that the logs could not, with numbers: `plans/2026-08-13_drill-06-request-id-propagation.md`.

`src/tenancy` owns tenant isolation. **Every table carrying `org_id` (`conversations`, `messages`, `memberships`) has row-level security enabled and one `FOR ALL … USING … WITH CHECK` policy** keyed on `app_current_org()`, a `STABLE PARALLEL SAFE` function reading a transaction-local `app.org_id`. `TenantDb.withOrg(orgId, fn)` is the only way to set it: one pinned client, `BEGIN` → `set_config(…, is_local => true)` → the callback → `COMMIT`. The API serves as `POSTGRES_APP_USER`, **not** the owner, and `PostgresService` throws at construction if it is unset. `GET /conversations` keeps an explicit `WHERE org_id = $1`; the four `/conversations/:id*` endpoints deliberately have **no** org filter and are scoped by the policies alone, which is what makes the removal proof real — don't "fix" them. Full reasoning, the rejected alternatives and the cost numbers: `plans/2026-08-15_drill-07-tenant-isolation.md`.
...
- **The skew is the point, and the two orgs to know are `1` and `150`.** Org 1 is the whale — 1,000,000 conversations, ~176ms requests. Org 150 is the tail — 2,631 conversations, ~2-3ms requests. Nine orgs hold ~111k each and the remaining 190 share the rest. Every measurement in drills 05 and 06 is `ORG_ID=1` or `ORG_ID=150`, and the two answer different questions: the whale finds query cost, the tail finds fixed per-request cost.
...
that the `/* rid= */` comment is on the wire. `trace:on` / `trace:off` start and stop the collector
and Jaeger together with the env var that enables the SDK — they must move together, because a set
endpoint with nothing listening is a retry loop rather than a trace. `trace:logs` is the
collector's own stdout, which answers "did the span arrive at all" without trusting the UI.
...
- **RLS is not enforced against a superuser, a `BYPASSRLS` role, or the table owner** (the last only without `FORCE ROW LEVEL SECURITY`). All three fail *silently and positively* — the policies are in the schema, `\d` shows them, every row comes back. The API connected as the superuser `postgres` from drill 01 to 06, which is why the app role exists. `pnpm check:tenancy` asserts all three.
...
- **A transaction-local GUC reverts to `''`, not unset.** Once a session has seen a custom setting's name, end-of-transaction restores its reset value, which for a never-session-set custom GUC is the empty string. So `current_setting('app.org_id', true)` is `NULL` only until a pooled connection's first scoped transaction, and `''` forever after — and `''::bigint` raises `22P02`. Hence `nullif(…, '')` inside `app_current_org()`: without it, fail-closed becomes a 500 on every unscoped query after the first.
...
- **SQL functions default to `PARALLEL UNSAFE`, and a policy calling one makes every query on that table serial.** Measured on the whale: `count(*)` 42.7 → 108.0ms, list page 1 88.9 → 214.7ms — **2.4x, from one missing word**. `STABLE` matters separately: it lets the planner fold the policy into a `One-Time Filter` (once per scan, not per row) and keeps the count query's index-only scan.
- **A role is a cluster object; `pgmigrations` is a database one.** `pnpm db:reset` (`DROP SCHEMA public CASCADE`) drops the tables, the function and the ledger but **leaves the role**, so migration 003 guards `CREATE ROLE` with a `pg_roles` check and re-runs its `GRANT`s unconditionally.
- **node-pg-migrate echoes every statement to stdout**, no flag involved — so `pnpm db:migrate` prints the app role's password. Fine for a local dev credential; in a real deployment the role is created out of band and the migration only grants.
...
- **Measurement rules, learned by getting them wrong.** Within one sweep the noise floor is 1.9–7.2%, but drill 05 ran the same sweep three times in one evening and the *medians* moved up to 14% — monotonically, the laptop drifting ~4% slower over 90 minutes. So: **interleave A/B arms in one sitting** rather than comparing across sittings, and treat a cross-sitting delta under ~15% (20% on the tail) as unproven. Drill 06 followed this and its within-arm spreads came in at 2–4%.
- **Arms must differ only in the variable, not in whether the process restarted.** Drill 07's `before` arm was the only one needing a whole-tree `git checkout`, so it was the only one restarting into a cold JIT and an empty pool: 33% spread across five runs against 1.6–5% for the other three arms, and it had to be discarded as a comparator. Build the "no mechanism" arm as a code path on the *same* commit instead.
...
- **OTel instrumentation patches modules as they load, so `import './tracing'` must be the literal first import of `main.ts`.** Below any other import the patches land on already-resolved modules: no error, no warning, no spans. `tracing.ts` therefore imports nothing from the app but two constants files — pulling in `logger.options.ts` would drag `nestjs-pino` and `@nestjs/core` in first.
- **Our `/* rid= */` SQL comment and instrumentation-pg's sqlcommenter are mutually exclusive.** `@opentelemetry/sql-common` refuses to comment a statement that already has one (the sqlcommenter spec says so), so the hand-rolled comment silently disables the standard `traceparent` one. Separately, instrumentation-pg names its span from the **first whitespace-delimited token**, so a *leading* comment renames every query span to `pg.query:/*`. Hence: comment trails, and is skipped entirely when tracing is on.
- **Next runs `proxy.ts` in its own trace** — a one-span `middleware GET` root, not a child of the render. So the request id cannot be minted from the active trace id at the edge; it would be valid, greppable, and point at the wrong trace. The API can do it (its span is the propagated server span); the web tier mints a UUID and `trace_id` on every log line is the join.
- **Next does not inject `traceparent` on outgoing `fetch`.** It creates an `AppRender.fetch` span but `patch-fetch.js` never writes the header, so `propagation.inject` is called by hand in `lib/api.ts`. Inbound on Nest is automatic (`instrumentation-http`). Also: the API's server span ends up a *sibling* of Next's fetch span, because the context is injected before `fetch()` is called.
- **The OTel packages need `serverExternalPackages` in `next.config.ts`.** Bundled, the SDK registers into a different copy of `@opentelemetry/api` than Next's own tracer holds — again, no error and no spans.
- **pnpm 11 fails `install --frozen-lockfile` on an undecided build script.** `sdk-node`'s gRPC exporters pull `protobufjs`; `allowBuilds: { protobufjs: false }` in `pnpm-workspace.yaml` is that decision. Without it the Docker image build fails, not the host install.
- **The collector's `debug` exporter at `verbosity: normal` prints one line per span** — ~10k lines/s into Docker's json-file driver at 1,800 req/s. Changed to `basic` (one line per batch, with a count) because that is all "did it arrive" needs. Worth recording that this was *suspected* of confounding the tracing A/B and, when measured, did not: the `normal` run came out marginally faster than the `basic` one. Hygiene, not the explanation.
...
- **Sampling is env-var only, no code:** `OTEL_TRACES_SAMPLER=parentbased_traceidratio` + `OTEL_TRACES_SAMPLER_ARG=0.05`. `parentbased_*` honours an inbound decision, which is what stops a trace being recorded by half its services.
