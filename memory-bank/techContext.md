...
- **`Promise.all` of two queries takes two pool connections per request.** `pool.query()` acquires and releases per call, so `GET /conversations` wants 2 of the pool's 10 per in-flight request — oversubscribed 2:1 at 10 VUs. Inference from timings, not yet observed: `PostgresService.stats()` exists but is not on a route.
